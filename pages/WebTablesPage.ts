...
export class WebTablesPage {
  ...
  async userAdd() {
    ...
    for (let i = 0; i < formLocator.length; i++) {
      await expect(this.page.locator(`#${formLocator[i]}`)).toBeEditable();
    }

    await this.page
      .locator(`#${formLocator[0]}`)
      .fill(WebTableData.row[3].firstName);
    await this.page
      .locator(`#${formLocator[1]}`)
      .fill(WebTableData.row[3].lastName);
    await this.page
      .locator(`#${formLocator[2]}`)
      .fill(WebTableData.row[3].email);
    await this.page
      .locator(`#${formLocator[3]}`)
      .fill(WebTableData.row[3].age.toString());
    await this.page
      .locator(`#${formLocator[4]}`)
      .fill(WebTableData.row[3].salary.toString());
    await this.page
      .locator(`#${formLocator[5]}`)
      .fill(WebTableData.row[3].department);
  ...
  async varifySerachButton(keyword: string) {
    ...
    await expect
      .soft(this.row.getByText(keyword, { exact: true }))
      .toBeVisible();
  }
}
