...
export class FramePage {
  ...
  async varifyFrame(){
    //  const totalFrame= await this.page.frames()
   // console.log("Total Frame Number =>>",totalFrame.length)
    await expect(this.frameHeadding1).toBeVisible()
    const frame2= await this.frameHeadding2;
